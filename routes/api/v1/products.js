...
router.post('/admin/create', isStaff, upload.single('image'), async (req, res) => {
    try {
        const { name, price, description, category, stock, product_code } = req.body;
        ...
        if (product_code) {
            ...
            if (existing) {
                return res.status(400).json({ message: 'Mã sản phẩm (SKU) đã tồn tại' });
            }
        }
        ...
        await db.query(
            ...
            [product_code ? product_code.trim() : null, name, price, imagePath, description || '', category, stock || 0]
        );
...
router.patch('/admin/:id', isStaff, upload.single('image'), async (req, res) => {
    try {
        const { name, price, description, category, stock, product_code } = req.body;
        ...
        await db.query(
            ...
            [
                product_code !== undefined ? (product_code.trim() || null) : existing.product_code,
                name || existing.name,
